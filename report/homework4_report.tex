...
Traditional computational fluid dynamics solves the macroscopic Navier-Stokes equations for mass and momentum conservation:
\begin{equation}
    \rho \left( \frac{\partial \vec{u}}{\partial t} + \vec{u} \cdot \nabla \vec{u} \right) = -\nabla p + \mu \nabla^2 \vec{u}
\end{equation}
When these fundamental conservation laws are solved for a fluid stream striking an unconfined bluff body with velocity $U$, the boundary layer separates due to pressure gradients. This structural instability manifests as a predictable wake pattern of alternating low-pressure regions known as a Von Kármán vortex street. 

The vortex shedding frequency ($f_v$) is dictated by the dimensionless Strouhal number ($St$). The frequency is defined as the product of the Strouhal number with the flow velocity divided by diameter $D$:
\begin{equation}
    f_v = \frac{St \cdot U}{D}
\end{equation}
These shedding vortices generate asymmetric, alternating low-pressure zones that exert a periodic transverse lift force on the pole perpendicular to the freestream flow.
